...
def get_house_list():
    ...
    if sort_key == "booking": # 入住最多
        # "*"号作用是将列表解开成两个独立的参数，传入函数，还有类似的有两个星号，是将字典解开成独立的元素作为形参。
    ...
    if page <= total_page:
        ...
        # 哈希类型
        try:
            # redis_key.hset(redis_key,page,resp_json)
            # redis_key.expire(redis_key,constants.HOUSE_LIST_PAGE_REDIS_CACHE_EXPIRES)
            ...
            pipeline.hset(redis_key,page_obj,resp_json)
