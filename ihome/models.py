...
class BaseModel(object):
    ...
    update_time=db.Column(db.DateTime,default=datetime.now,onupdate=datetime.now)# 订单的更新时间
...
class User(BaseModel,db.Model):
    ...
    def password(self,value):
        '''
        设置属性  如果user.password='xxxxx'  value就是'xxxxx'
    ...
    def to_full_dict(self):
        ...
        for order in orders:
            comment={
                ...
                "user_name":order.user.name if order.user.name !=orders.user.mobile else "匿名用户", # 发表评论的用户
